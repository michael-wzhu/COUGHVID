...
from src.TimmSED_v1 import metric
from src.TimmSED_v1 import zoo_transforms

from src.TimmSED_v1.config_utils import load_config
from src.TimmSED_v1.losses import tn_score, tp_score
from src.TimmSED_v1.dataset_loader import AudioDataset
from src.TimmSED_v1.trainer import Evaluator, PytorchTrainer, TrainConfiguration
...
class AudioEvaluator(Evaluator):
    ...
    def validate(self, dataloader: DataLoader,
        ...
        for sample in tqdm(dataloader):
            mel_spect = sample["mel_spect"].cuda()
            labels = sample["labels"].numpy()

            model.eval()
            outs = model(mel_spect, is_test=True)
...
def parse_args():
    ...
    arg('--workers', type=int, default=2, help='number of cpu threads to use PER GPU!')
...
if __name__ == '__main__':
    ...
    # 进行批量的预测
    # python src/TimmSED/train_classifier.py --data_dir ./datasets/covid19-cough/raw --folds_csv src/TimmSED/data_process/folds_covid19_coughs.csv --resume weights/val_TimmClassifier_v1_eca_nfnet_l1_0_f1_score --do_eval
